...
class Program
{
    ...
    class Stat : System.IComparable<Stat>
    {
        ...
        internal Stat( int x, int y, int c, Stat s )
        {
            Cost = c;
            X = x;
            Y = y;
    ...
    static void Main()
    {
        ...
        while( 0 < q.Count ) {
            ...
            foreach( int i in Range( 0, 4 ) ) {
                ...
                if( 0 <= x && x < hw[1] && 0 <= y && y < hw[0] ) {
                    q.Enq( new Stat( x, y, p.Cost + t[y][x], p ) );
                }
            }
        }
        WriteLine( st.Cost );
        while( st != null ) {
            foreach( int i in Range( 0, hw[0] ) ) {
                foreach( int j in Range( 0, hw[1] ).Where( x => i == st.Y && x == st.X ) ) {
                    WriteLine( $"{i} {j}" );
                }
            }
            st = st.Ref;
